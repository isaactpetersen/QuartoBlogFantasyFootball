---
title: "Example Blog Post: Are Air Yards More Predictive than Actual Receiving Yards of Future Fantasy Points?"
...
Due to natural variability of players' fantasy points, it is possible that opportunity may be a stronger influence of future performance than previous performance.
However, this possibility has not been tested.
In the present study, I examined whether previous performance or opportunity is a stronger predictor of future performance among wide receivers.
I examined air yards as the index of a wide receiver's opportunity; I examined fantasy points as the index of player performance.
I found that previous performance was a stronger predictor of fantasy performance than was previous opportunity.
Nevertheless, both previous fantasy points and air yards accounted for unique variance in future fantasy points.
Both previous performance and opportunity are important predictors of fantasy performance and should be considered when predicting wide receiver fantasy performance.

# Background {#sec-background}

A key predictor of future performance in football is past performance.
Wide receivers who score many points in a given week or season are more likely than other players to score many points in future weeks or seasons.
...
It is common for players to [regress to the mean](https://www.footballguys.com/article/stuart_regression_to_the_mean) (Stewart, 2013), where players who do not meet expectations in Week 1 or 2 may rebound in subsequent weeks; and players who outperform their expectations may be a blip on the radar and revert to their expected performance.
After several weeks into the season, players' situations tend to become better understood and thus their performance may become more predictable (i.e., their expected mean may change).
...
A key index of the length of the targets that a player receives is called "air yards" (Hermsmeyer, 2018).

[*Air yards*](https://bradcongelio.com/nfl-analytics-with-r-book/a1-nfl-analytics-dictionary.html#air-yards) on a given play are defined as the yardage distance the ball travels in air, from the line of scrimmage, to the point the receiver catches or does not catch the ball (Congelio, 2023).
Air yard distance is tracked in yards from the line of scrimmage, not the true distance the ball travels in air (Hermsmeyer, 2018).
...
Receiving air yards over the course of a game is the sum of air yards in that game for all passes targeting the player.
Receiving air yards thus represent an important index of a wide receiver's opportunity, volume, or usage (Hermsmeyer, 2018).
Moreover, among underperforming players, considering their air yards may be useful for identifying the ones who might bounce back in subsequent weeks due to regression to the mean.
...
In the present study, I examined whether previous performance or opportunity is a stronger predictor of future performance among wide receivers.
I operationalized a wide receiver's performance using their fantasy points, calculated by Petersen (2025) using the `ffanalytics` package (Andersen, Petersen, & Tungate, 2025).
I operationalized a wide receiver's opportunity using their air yards, obtained from the `nflreadr` package (Ho & Carl, 2024).
I examined the wide receiver's fantasy points and air yards in a given week in predicting their fantasy points in the subsequent week.
...
I hypothesized that, due to natural variability from week-to-week that, in part, reflects randomness, opportunity would be a stronger influence of subsequent performance than previous performance.
...
To examine the hypothesis, I examined whether receiving air yards in a given week was a better predictor than actual receiving yards for a wide receiver's fantasy points the following week.
...
I obtained the data file from the [Open Science Framework repository](https://osf.io/z6pg4) for the textbook, "Fantasy Football Analytics: Statistics, Prediction, and Empiricism Using R" (Petersen, 2025).
...
The lagged variable of fantasy points has fewer observations than the original fantasy points variable because, as I mentioned earlier, I lagged data within seasons; consequently, week 1 data were not lagged to the prior season and thus were not retained in the lagged variable.
...
Specifically, I compared the standardized regression coefficients for the two predictors of interest: receiving air yards versus receiving yards.

# Results {#sec-results}

## Main Analyses

@fig-associationAirYardsFantasyPoints depicts the association between air yards and fantasy points among wide receivers.
...
ggplot2::ggplot(
  ...
  coord_cartesian(
    ...
    expand = FALSE) +
  labs(
    x = "Receiving Air Yards (Game)",
    y = "Fantasy Points in the Following Week",
    title = "Fantasy Points by the Previous Week Receiving Air Yards",
    subtitle = "(Among Wide Receivers)"
  ) +
  theme_classic()
```

@fig-associationReceivingYardsFantasyPoints depicts the association between receiving yards and fantasy points among wide receivers.
...
The standardized regression coefficient for receiving air yards was larger than for receiving yards.
As shown in @fig-regressionCoefficientsStandardized, their 95% confidence intervals did not overlap, indicating that the standardized regression coefficient for receiving air yards was larger than for receiving yards.
The standardized regression coefficient for receiving air yards in predicting fantasy points was $\beta = `r petersenlab::apa(beta_airYards, decimals = 2)`$.
...
A histogram of the transformed variable is in @fig-histogram.
...
hist(
  analysisData$fantasyPoints_lag_transformed,
...
Because the data were nested with players (i.e., longitudinal) and thus violated the assumption of uncorrelated residuals, we also conducted a sensitivity analysis using a mixed model to account for the nested data.
...
To operationalize fantasy performance, I examined the wide receiver's fantasy points.
To operationalize opportunity, I examined air yards, an index of the depth of targets the wide receiver received.
...
Counter to my hypothesis, however, I found that previous fantasy points was a stronger predictor than air yards in predicting wide receivers' fantasy points.
Nevertheless, both previous fantasy points and air yards accounted for unique variance in future fantasy points.
Our findings held when conducting sensitivity analyses, including transforming the outcome variable to be more normally distributed and using a mixed model to account for the longitudinal nature of the data.
Thus, findings suggest that, although previous performance is a stronger predictor than previous opportunity in predicting future fantasy performance among wide receivers, both are important predictors of fantasy performance and should be considered when predicting wide receiver fantasy performance.

## Strengths and Limitations {#sec-strengthsLimitations}

The study had several strengths.
First, I examined data from all seasons since 1999, thus providing strong statistical power to detect differential associations.
Second, I conducted several sensitivity analyses, providing additional rigor and confidence in the findings.
...
Among underperforming players, air yards may be a useful way of identifying those who may bounce back in subsequent weeks.
